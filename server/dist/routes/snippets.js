...
import { createSnippetSchema, updateSnippetSchema, snippetParamsSchema, listSnippetsQuerySchema, detectLanguage } from '../middleware/validation.js';
const router = Router();
...
router.get('/', validateQuery(listSnippetsQuerySchema), async (req, res) => {
    const { limit, offset, search, language, collection_id, tag, sort_by, sort_order } = req.query;
    try {
        ...
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) FROM snippets s ${whereClause}`, params);
...
router.post('/', validateBody(createSnippetSchema), async (req, res) => {
    const { title, description, code, language, collection_id, tag_ids, is_public, filename } = req.body;
...
router.post('/:id/copy', validateParams(snippetParamsSchema), async (req, res) => {
    try {
        const result = await pool.query('SELECT code, title FROM snippets WHERE id = $1', [req.params.id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                error: {
                    code: 'NOT_FOUND',
...
router.delete('/:id/tags/:tagId', validateParams(z.object({
    ...
    try {
        const result = await pool.query('DELETE FROM snippet_tags WHERE snippet_id = $1 AND tag_id = $2 RETURNING *', [id, tagId]);
        if (result.rowCount === 0) {
            return res.status(404).json({
                error: {
                    code: 'NOT_FOUND',
